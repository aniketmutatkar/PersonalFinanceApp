...
async def get_monthly_summaries(
    ...
    """
    Get all monthly summaries with optional year filter
    """
    try:
        if year:
            # Get summaries for specific year
            summaries = monthly_summary_repo.find_by_year(year)
        else:
            # Get all summaries
...
async def get_monthly_summary(
    ...
    """
    Get a specific monthly summary by month_year (e.g., "January 2023")
    """
    try:
